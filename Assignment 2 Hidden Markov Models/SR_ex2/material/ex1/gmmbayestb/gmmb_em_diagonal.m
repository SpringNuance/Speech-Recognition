%GMMB_EM_DIAGONAL    - EM estimated GMM parameters with diagonal covariance
%
% estS = gmmb_em_diagonal(data)
% estS = gmmb_em_diagonal(data, <params>...)
% [estS, stats] = gmmb_em_diagonal(...)
%
...
%
% Example:
%   estS = gmmb_em_diagonal(data, 'init', 'fcm1', 'components', 5, 'thr', 1e-8)
%
% References:
...
%
%   Modified from GMMB_EM for T-61.5150
%
...
function [estimate, varargout] = gmmb_em_diagonal(data, varargin);
...
% N_limit = (Nparc+1)*3*C;
% if N < N_limit
% 	warning_wrap('gmmb_em_diagonal:data_amount', ...
...
% Convert initial covariance matrices to diagonal
...
while 1
	...
	for c = 1:C
		...
		nsigma = diag(sum(moddata.^2) ./ psum);
	...
	if fixing_cycles > 20
		warning_wrap('gmmb_em_diagonal:fixing_loop', ...
